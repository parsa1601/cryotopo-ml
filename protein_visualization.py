...
class ProteinVisualizer:
    ...
    def plot_3d_cylindrical_structures_with_svm(self, X_train, y_train, protein_name, structure_type="Helix", svm_model=None, title_suffix=""):
        """
        ...
        print(f"Creating 3D cylindrical visualization for {protein_name} with {len(unique_classes)} {structure_type.lower()}s{model_info}")
        
        # Plot cylindrical structures
        ...
        # Add SVM decision boundaries if model is provided
        if svm_model is not None:
            try:
                ...
                # Add some annotations for SVM info
                for i in range(0, len(X_train), max(1, len(X_train)//30)):  # Show fewer labels to avoid clutter
                    ax.text(X_train[i, 0], X_train[i, 1], X_train[i, 2], 
                           str(y_train[i]), fontsize=6, alpha=0.4)
    ...
    def _plot_decision_boundaries(self, ax, X_train, svm_model):
        ...
        """
        try:
            # Get data bounds for boundary visualization
            ...
            if hasattr(svm_model, 'coef_') and hasattr(svm_model, 'intercept_'):
                ...
                # Plot multiclass hyperplanes (handles both binary and multi-class)
                self._plot_multiclass_hyperplanes(ax, coef, intercept, x_min, x_max, y_min, y_max, z_min, z_max)
                
        except Exception as e:
            print(f"Could not plot decision boundaries: {e}")
    
    
    def _plot_multiclass_hyperplanes(self, ax, coef_matrix, intercept_vector, x_min, x_max, y_min, y_max, z_min, z_max):
        """
        Plot up to 5 hyperplanes for classification.
        """
        colors = ['red', 'blue', 'green', 'orange', 'purple']
        
        # Limit to maximum 5 hyperplanes
        ...
        for i in range(max_hyperplanes):
            ...
            # Create a mesh for each hyperplane
