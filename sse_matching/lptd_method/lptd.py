...
class LPTDMethod:
    ...
    def run(self, helices_datapoints, cryo_datapoints, classes, sticks, mode="Helix"):
        """
        ...
        for sid in stick_ids:
            ...
            if mode == "Strand":
                # Generate more voxels for strand sticks using Bresenham
                # The MATLAB code takes the first and last point?
                # p1=stick_voxels_strands(j).Mat(1,:)
                # p2=stick_voxels_strands(j).Mat(3,:) -> Wait, index 3?
                # It assumes there are at least 3 points?
                # And it replaces the points with the line?
                # Let's look at MATLAB code:
                # p1=stick_voxels_strands(j).Mat(1,:)
                # p2=stick_voxels_strands(j).Mat(3,:)
                # [x,y,z]=bresenham_line3d(p1,p2)
                # generated_stick_voxels(j).Mat(:,1)=x ...
                
                # If points has < 3 rows, this might fail or be weird.
                # Assuming points are sorted or ordered.
                # I'll use the first and last point of the stick points provided.
                if len(points) >= 2:
                    p1 = points[0]
                    p2 = points[-1]
                    # Or maybe it expects exactly 3 points for sticks in input?
                    # "stick_Matrix(k,:)=d(i,1:3);"
                    # It reads all rows for that stick.
                    # If the input CSV for sticks has only start/end, then it's fine.
                    # But if it has many points, taking 1 and 3 is specific.
                    # I'll assume start and end (0 and -1).
                    
                    # Wait, MATLAB code: p2=stick_voxels_strands(j).Mat(3,:)
                    # This implies the stick input has at least 3 rows.
                    # If I look at the file structure, maybe sticks are defined by 3 points?
                    # I'll use 0 and -1 to be safe for general line.
        ...
        for item in final_topology:
            ...
            if sid != 0:
                ...
                if dist1 <= dist2: # MATLAB: < for helix, <= for sheet? 
                    # Helix: if dtw_distance1<dtw_distance2 direction=1 else -1
                    # Sheet: if dtw_distance1<=dtw_distance2 direction=1 else -1
                    # I'll use <= for both for simplicity or strictly follow
                    if mode == "Helix":
